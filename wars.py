...
def war_embed(war_id: int) -> discord.Embed:
    ...
    lines = [f"**{i}.** <@{uid}>" for i, uid in enumerate(parts, 1)]
    if not lines:
...
class RecruitView(discord.ui.View):
    ...
    async def join_callback(self, interaction: discord.Interaction):
        ...
        if not war or war["status"] != "recruiting":
            return await interaction.response.send_message("Війна вже не набирає учасників.", ephemeral=True)
        current = participant_ids(self.war_id)
        if interaction.user.id in current:
            return await interaction.response.send_message("Ти вже записаний/а.", ephemeral=True)
        if war["participant_limit"] and len(current) >= war["participant_limit"]:
            return await interaction.response.send_message("Ліміт учасників уже заповнений.", ephemeral=True)
        db.conn.execute(
            "INSERT INTO war_participants(war_id,guild_id,user_id,status,created_at) VALUES(?,?,?,?,?)",
            (self.war_id, GUILD_ID, interaction.user.id, "registered", now_iso()),
        )
        db.conn.commit()
...
def resource_totals(owner_name: str = "Agosto", period: bool = False) -> dict:
    ...
    rows = db.conn.execute(
        "SELECT * FROM war_resource_ledger WHERE guild_id=? AND owner_name=?" + (" AND ended_at >= ?" if start else ""),
        (GUILD_ID, owner_name, start) if start else (GUILD_ID, owner_name),
    ).fetchall()
    totals = {"money": 0, "rating": 0, "authority": 0}
    for row in rows:
        for key in totals:
            totals[key] += int(row[key])
    # Include currently open ownership without closing it.
    opens = db.conn.execute(
        "SELECT * FROM war_ownership_periods WHERE guild_id=? AND owner_name=? AND closed=0",
        (GUILD_ID, owner_name),
    ).fetchall()
    ...
    for row in opens:
        ...
        if period and start:
            started = max(started, parse_dt(start))
...
def build_stats_embed() -> discord.Embed:
    ...
    embed.add_field(name="📅 Поточний період", value=f"💵 {fmt_money(cur['money'])}\n🏆 {cur['rating']} рейтингу\n⭐ {cur['authority']} авторитету", inline=False)
    embed.add_field(name="📊 За весь час", value=f"💵 {fmt_money(total['money'])}\n🏆 {total['rating']} рейтингу\n⭐ {total['authority']} авторитету", inline=False)
    return embed
...
def rating_rows(period: bool = True):
...
def build_profile(user_id: int) -> discord.Embed:
    def one(period):
        rows = [r for r in rating_rows(period) if int(r["user_id"]) == user_id]
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            cur = db.conn.execute(
                ...
                (GUILD_ID, name, owner, now_iso(), now_iso()),
            )
            complex_id = cur.lastrowid
            db.conn.commit()
            begin_ownership(complex_id, owner)
...
class CommanderView(discord.ui.View):
    ...
    @discord.ui.button(label="Створити війну", emoji="➕", style=discord.ButtonStyle.success)
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_commander(interaction.user):
            return await interaction.response.send_message("Тільки командир або керівництво.", ephemeral=True)
        await interaction.response.send_modal(CreateWarModal())

    @discord.ui.button(label="Історія", emoji="📜", style=discord.ButtonStyle.secondary, row=1)
    async def history(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_commander(interaction.user):
            return await interaction.response.send_message("Тільки командир або керівництво.", ephemeral=True)
        await interaction.response.send_message(embed=build_history_embed(), ephemeral=True)

    @discord.ui.button(label="Статистика", emoji="📊", style=discord.ButtonStyle.secondary, row=1)
    ...
    @discord.ui.button(label="Керувати війною", emoji="🎛️", style=discord.ButtonStyle.secondary, row=1)
    async def manage(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_commander(interaction.user):
            return await interaction.response.send_message("Тільки командир або керівництво.", ephemeral=True)
        await interaction.response.send_message("Вкажи ID війни командою `/wars-manage`.", ephemeral=True)

    @discord.ui.button(label="Налаштування системи", emoji="⚙️", style=discord.ButtonStyle.primary)
...
async def publish_panels(bot) -> None:
    if WARS_CHANNEL_ID:
        await send_to_channel(bot, WARS_CHANNEL_ID, embed=discord.Embed(
    ...
    if WARS_MANAGEMENT_CHANNEL_ID:
        await send_to_channel(bot, WARS_MANAGEMENT_CHANNEL_ID, embed=discord.Embed(
            title="⚔️ Керування війнами за комплекси",
            description="Створення та керування війнами доступне командирам. Налаштування системи — керівництву.",
...
def register_commands(bot) -> None:
    ...
    async def wars_setup(interaction: discord.Interaction):
        if not is_management(interaction.user):
            return await interaction.response.send_message("Тільки керівництво.", ephemeral=True)
        await publish_panels(interaction.client)
        await interaction.response.send_message("✅ Панелі системи війн опубліковано.", ephemeral=True)
    ...
    async def wars_add_member(interaction: discord.Interaction, war_id: int, member: discord.Member):
        ...
        if member.id in ids:
            return await interaction.response.send_message("Цей учасник уже записаний.", ephemeral=True)
        if war["participant_limit"] and len(ids) >= war["participant_limit"]:
            return await interaction.response.send_message("Ліміт учасників уже заповнений.", ephemeral=True)
        db.conn.execute(
            "INSERT INTO war_participants(war_id,guild_id,user_id,status,created_at) VALUES(?,?,?,?,?)",
            (war_id, GUILD_ID, member.id, "registered", now_iso()),
        )
        db.conn.commit()
    ...
    async def wars_complete(interaction: discord.Interaction, war_id: int, result: str, mvp_user_id: Optional[str] = None, comment: Optional[str] = None):
        ...
        if result in ("перемога", "win", "победа"):
            ...
            return await interaction.response.send_message("Результат: `win` або `loss`.", ephemeral=True)
        mvp = None
        if mvp_user_id:
            try:
                mvp = int(mvp_user_id)
            except ValueError:
                return await interaction.response.send_message("MVP має бути Discord ID.", ephemeral=True)
        ids = participant_ids(war_id)
        if mvp is not None and mvp not in ids:
            return await interaction.response.send_message("MVP має бути учасником цієї війни.", ephemeral=True)
        ...
        # Attack winner takes the complex; defense winner keeps it.
        if (war["attack_type"] == "attack" and result == "win") or (war["attack_type"] == "defense" and result == "loss"):
            ...
            # Close the existing ownership interval even if the owner stays the same,
            # then start a new interval so the exact war-end timestamp is preserved.
            complex_row = get_complex(war["complex_id"])
            if complex_row:
                current_owner = complex_row["owner_name"]
                settle_ownership(war["complex_id"], ended)
                begin_ownership(war["complex_id"], current_owner, ended)
        db.conn.commit()
